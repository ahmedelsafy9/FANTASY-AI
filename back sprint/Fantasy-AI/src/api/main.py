...
def create_app() -> FastAPI:
    ...
    Returns:
        FastAPI: The configured application, with routers and a
        ``/swagger`` docs endpoint registered.
    """
    settings = get_settings()
    app = FastAPI(
    ...
    def health() -> HealthResponse:
        ...
        Returns:
            HealthResponse: Status, and (if loaded) the active model
    ...
    async def require_state_loaded(request, call_next):
        """Return 503 for data-dependent routes if startup state failed to load.
        ...
        """
        always_available_paths = ("/", app.docs_url, app.redoc_url, "/openapi.json")
        if request.url.path not in always_available_paths and getattr(
            app.state, "fantasy_ai_state", None
        ) is None:
            return JSONResponse(
                status_code=503,
