...
**Fixture-aware roadmap (Phases 1-5)**: implemented on top of the
completed sprints. See [`FIXTURE_AWARE_ROADMAP.md`](./FIXTURE_AWARE_ROADMAP.md)
for the full writeup: the `opponent_strength` team-ID domain bug is
fixed, a new `team_form_trend` leakage-safe feature was added, next-
Gameweek prediction now uses real upcoming fixtures where reachable
(with an honest `fixture_source` flag when it falls back to the
proxy), a statistically-grounded (never fabricated) per-prediction
uncertainty metric was added for tree-ensemble models, and a
presentation-metadata layer (player photos, team badges) was built
fully separate from the ML pipeline.
