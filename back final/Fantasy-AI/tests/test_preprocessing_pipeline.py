...
def test_build_default_pipeline_steps_returns_seven_steps() -> None:
    """The factory must build the full standard step sequence, including both
    fixture-based and ID-based opponent resolution steps."""
    steps = build_default_pipeline_steps(PreprocessingSettings(), ValidationSettings())
    names = [step.name for step in steps]
    assert names == [
        "normalize_names",
        "resolve_opponent_from_fixture",
        "normalize_opponent_id",
        "drop_duplicates",
